...
from Teruati.modules.helper_funcs.string_handling import markdown_parser

import Teruati.modules.sql.feds_sql as sql

from Teruati.modules.helper_funcs.alternate import (
    send_message,
    typing_action,
    send_action,
)
from Teruati.modules.helper_funcs.decorators import kigcmd, kigcallback
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "Teruati_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="Teruati_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "Teruati_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="Teruati_fbanned_users.csv",
...
def fed_help(update: Update, context: CallbackContext):
    ...
    query.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            ...
            InlineKeyboardButton(text='Report Error', url='https://t.me/TeruatiSupports')]]
        ),
    )

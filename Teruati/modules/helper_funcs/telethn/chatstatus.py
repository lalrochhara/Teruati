from Teruati.modules.helper_funcs.telethn import HIGHER_AUTH, telethn
from Teruati import SUPPORT_USERS, SARDEGNA_USERS, WHITELIST_USERS
...
async def Teruati_is_admin(chat_id: int):
    status = False
    Teruati = await telethn.get_me()
    async for user in telethn.iter_participants(
            chat_id, filter=ChannelParticipantsAdmins):
        if Teruati.id == user.id:
            status = True
            break
    return status
